...
struct hash{    //Tabela;
    ...
    unsigned int n; //Quantidade de elementos contidos no vetor;
    ...
    //laço responsável pelo tamanho do vetor;
     for (int i = 25; i<=5000;i+=25){    //vetor é aumentado a cada loop +25vezes até 5000;
        ...
        for (int  j = 0; j<i; j++){ //estrutura criada de acordo com o tamanho da variável 'i' ;
            hinSert(&H,v[j]);   //chamacda da função de inserção, *é passado o endereço do Hash, e o valor a ser inserido;
        }
        ...
        for(int j = 0; j<5000;j++){ //o laço será percorrido 5000 vezes;
            ...
            buscaChave(H, rand()%10000); //chamada para a função de busca *recebe conteúdo de R e valor a ser buscado para que ocorra o melhor caso;
...
//Função para conferir a existenciac de números repetidos no vetor;
int confere(int v[], int n){    //*vetor/valor;

    for (int i = 0; i<5000; i++){   //loop do tamanho do vetor;
       if (v[i] == n){  //se a posição 'i' do vetor for igual ao valor 'n';
           break;   //saia do loop e gere um novo número aleatório;
       }
       if(v[i] == -1){   //se a posição 'i' do vetor for igual a -1, subentende-se que não há conteúdo e insere-se o valor;
            return n;
       }
    }

    return confere(v, rand()%10000);    //chama a mesma função e insere um novo valor aleatório;
}
